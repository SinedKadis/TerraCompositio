...
public class InfuserBehaviour extends PPOutputBehaviour{

    private final int range = 5;
    List<LivingEntity> entityList = List.of();

    public InfuserBehaviour(PathPointerBlockEntity blockEntity) {
        super(blockEntity);
    }
    ...
    public void onUpdate() {
        ...
        scanCompatibleEntities().forEach(livingEntity ->
                livingEntity.getCapability(TCCapabilities.CFE).ifPresent(icfeHandler ->
                        TCUtil.tryCFETransfer(icfeHandler, cfeHandler, icfeHandler.getFreeSpace())));
    }
