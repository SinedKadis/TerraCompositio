...
public class FlowCedarCasingBlockEntityRenderer implements BlockEntityRenderer<FlowCedarCasingBlockEntity> {
    public FlowCedarCasingBlockEntityRenderer(BlockEntityRendererProvider.Context context) {

    }


    @Override
    public void render(FlowCedarCasingBlockEntity pBlockEntity, float pPartialTick, @NotNull PoseStack pPoseStack, @NotNull MultiBufferSource pBuffer, int pPackedLight, int pPackedOverlay) {
...
//        ItemStack itemStack4 = pBlockEntity.getRenderStack();
        ...
        if (FlowCedarCasingBlock.isPortAttached(pBlockEntity.getLevel(),blockState,pBlockEntity.getBlockPos())) {
            pPoseStack.pushPose();
            switch (facing) {
            ...
            if (level != null) {
                itemRenderer.renderStatic(itemStack, ItemDisplayContext.FIXED, getLightLevel(level, pBlockEntity.getBlockPos(),facing),
