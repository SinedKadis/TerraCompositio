...
import static net.sinedkadis.terracompositio.block.entity.renderer.FlowExtractorBlockEntityRenderer.Model3D.DIRECTIONS;
...
public class RenderResizableCuboid {
    ...
    public static void renderCube(FlowExtractorBlockEntityRenderer.Model3D cube, PoseStack matrix, VertexConsumer buffer, int argb, int light, int overlay, FaceDisplay faceDisplay, Camera camera,
     ...
     */
    public static void renderCube(FlowExtractorBlockEntityRenderer.Model3D cube, PoseStack matrix, VertexConsumer buffer, int[] colors, int light, int overlay, FaceDisplay faceDisplay, Camera camera,
                                  @Nullable Vec3 renderPos) {
        TextureAtlasSprite[] sprites = new TextureAtlasSprite[6];
        int axisToRender = 0;
        if (renderPos != null && faceDisplay != FaceDisplay.BOTH) {
            GLOGGER.debug("RENDER: "+"renderPos is not null, faceDisplay is both");
            ...
            for (Direction direction : DIRECTIONS) {
                ...
                if (sprite != null) {
                    GLOGGER.debug("RENDER: "+"sprite not null: {}",sprite);
                    ...
                    if (faceDisplay.front == (axisDirection == AxisDirection.POSITIVE)) {
                       // GLOGGER.debug("RENDER: "+"axisDir is positive and it's equals front face display");
            ...
            GLOGGER.debug("RENDER: "+"renderPos i null or faceDisplay isn't both");
            for (Direction direction : DIRECTIONS) {
                TextureAtlasSprite sprite = cube.getSpriteToRender(direction);
                if (sprite != null) {
                    GLOGGER.debug("RENDER: "+"sprite is not null: {}",sprite);
                    sprites[direction.ordinal()] = sprite;
