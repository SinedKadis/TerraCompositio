...
public class CFETrashCanBlockEntity extends ModCFEBlockEntity {

    public CFETrashCanBlockEntity(BlockPos pPos, BlockState pBlockState) {
        super(ModBlockEntities.CFE_TRASH_CAN_BE.get(),pPos, pBlockState,Integer.MAX_VALUE,10,BlockMode.CONSUMER);
    ...
    public void onCFENetworkMemberUpdate() {
        TerraCompositioAPI.instance().getCFENetworkInstance().getAllCFENetworkMembers(level).stream()
                ...
                .filter(cfeNetworkMemberBE -> ((ModCFEBlockEntity) cfeNetworkMemberBE.getBE()).getCfeContainer().getCFE() > 0)
                ...
                .forEach(cfeNetworkMemberBE -> {
                    ...
                    if (level != null) {
                        if (level instanceof ServerLevel serverLevel){
                            TCUtil.sendCFEParticles(serverLevel,worldPosition,cfeNetworkMemberBE.getBlockPos(),transfer);
                        }
                    }
                });
    }
